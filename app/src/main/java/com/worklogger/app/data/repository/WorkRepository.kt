...
import com.worklogger.app.data.local.AdvancePurchaseDao
...
class WorkRepository(
    ...
    private val advancePurchaseDao: AdvancePurchaseDao
) {
    ...
    val allPurchaseRecords: Flow<List<AdvancePurchaseRecord>> = advancePurchaseDao.getAllPurchaseRecords()
    
    val totalPurchaseAmount: Flow<Double> = advancePurchaseDao.getTotalPurchaseAmount()
    
    val trashPurchaseRecords: Flow<List<AdvancePurchaseRecord>> = advancePurchaseDao.getTrashRecords()
    
    suspend fun insertPurchaseRecord(record: AdvancePurchaseRecord) {
        advancePurchaseDao.insert(record)
    }
    
    suspend fun updatePurchaseRecord(record: AdvancePurchaseRecord) {
        advancePurchaseDao.update(record)
    }
    
    suspend fun deletePurchaseRecord(record: AdvancePurchaseRecord) {
        advancePurchaseDao.delete(record)
    }
    
    suspend fun movePurchaseToTrash(id: Int) {
        advancePurchaseDao.moveToTrash(id.toLong())
    }
    
    suspend fun restorePurchaseFromTrash(id: Int) {
        advancePurchaseDao.restoreFromTrash(id.toLong())
    }
    
    suspend fun permanentlyDeletePurchase(id: Int) {
        advancePurchaseDao.permanentlyDelete(id.toLong())
    }
    
    suspend fun getPurchaseRecordsByMonth(year: Int, month: Int): List<AdvancePurchaseRecord> {
        val startDate = String.format("%04d-%02d-01", year, month)
        val endDate = if (month == 12) {
            String.format("%04d-01-01", year + 1)
        } else {
        ...
        return advancePurchaseDao.getPurchaseRecordsByDateRange(startDate, endDate)
    }
    
    suspend fun getPurchaseAmountByMonth(year: Int, month: Int): Double {
        val startDate = String.format("%04d-%02d-01", year, month)
        val endDate = if (month == 12) {
            String.format("%04d-01-01", year + 1)
        } else {
        ...
        return advancePurchaseDao.getPurchaseAmountByDateRange(startDate, endDate)
    }
    
    suspend fun getTotalPurchaseAmountOnce(): Double {
        return advancePurchaseDao.getTotalPurchaseAmountOnce()
    }
    
    suspend fun getAllPurchaseLocations(): List<String> {
        return advancePurchaseDao.getAllPurchaseLocations()
    }
    
    val recentPurchaseLocations: Flow<List<String>> = allPurchaseRecords.map { records ->
        records.map { it.location }.filter { it.isNotBlank() }.distinct().sorted()
    }
    
    suspend fun deleteAllPurchaseRecords() {
